...
// ============================================================
// Mic level metering (visual only) while listening
// ------------------------------------------------------------
// Web Speech API does its own mic capture internally and doesn't
// expose the raw audio, so we open a second, independent mic stream
// purely to drive the orb's reactive glow — no audio data from this
// stream goes anywhere.
// ============================================================
...
async function startTurn() {
  ...
  try {
    ...
    console.error(err);
    showError("Couldn't access your microphone. Check browser permissions and try again.");
    resetToIdle();
    return;
  }
  ...
  recognition.onend = () => {
    // If recognition ended without ever firing onresult (silence timeout),
    // make sure we don't get stuck in "listening".
